...
function saveSession(result:{access_token:string;user:User}) { token=result.access_token;user=result.user;sessionStorage.setItem('dentara_token',token);sessionStorage.setItem('dentara_user',JSON.stringify(user)); }
async function boot(){
...
el<HTMLFormElement>('#login-form').addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget;const button=form.querySelector('button[type="submit"]') as HTMLButtonElement;button.disabled=true;el('#login-error').hidden=true;try{const data=new FormData(form);const params=new URLSearchParams({username:String(data.get('username')),password:String(data.get('password'))});const result=await api('/auth/login',{method:'POST',body:params});saveSession(result);await boot();}catch(e){el('#login-error').textContent=(e as Error).message;el('#login-error').hidden=false;}finally{button.disabled=false;}});
els<HTMLButtonElement>('[data-demo]').forEach(button=>button.onclick=async()=>{if(['localhost','127.0.0.1'].includes(location.hostname)){el<HTMLInputElement>('#login-form input[name="username"]').value=button.dataset.demo!;el<HTMLInputElement>('#login-form input[name="password"]').focus();return;}els<HTMLButtonElement>('[data-demo]').forEach(x=>x.disabled=true);el('#login-error').hidden=true;try{const result=await api(`/auth/demo/${button.dataset.role}`,{method:'POST'});saveSession(result);await boot();}catch(e){el('#login-error').textContent=(e as Error).message;el('#login-error').hidden=false;}finally{els<HTMLButtonElement>('[data-demo]').forEach(x=>x.disabled=false);}});
